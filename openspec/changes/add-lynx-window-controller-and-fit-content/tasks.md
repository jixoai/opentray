...
- [ ] 2.5 Scenario: Given `fitContentSize` is left unspecified When a Lynx bundle is shown with no explicit fixed size Then the first visible window size follows fit-content policy instead of a large arbitrary fixed shell.
- [ ] 2.6 Scenario: Given `fitContentSize: false` or explicit `width` / `height` are provided When the window launches Then fixed host sizing wins and content-fit does not override those explicit dimensions.
- [ ] 2.7 Scenario: Given fit-content computes a size outside configured bounds When min/max limits are present Then the final frame is clamped and emitted events report the clamped size.
...
- [x] 3.2 Extend `@opentray/ext-lynx` public types and commands so `show` can configure native window API, native screen API, initial title/icon, fit-content policy, fixed size, and sizing bounds.
- [x] 3.3 Extend `crates/opentray-ext-lynx` command parsing and event emission for host-window control, title/icon metadata, screen details, capability queries, style state, and fit-content lifecycle.
...
- [x] 3.6 Implement default-on fit-content sizing with explicit opt-out, explicit fixed-size precedence, and min/max clamp behavior for standalone Lynx windows.
...
- [x] 3.8 Add concise intent comments at the critical effect points for host bridge ownership, fit-content defaulting, and fixed-size precedence.
- [x] 3.9 Update Lynx smoke/demo surfaces so a human can verify the controller, metadata, screen, and fit-content behavior visually instead of only reading logs.
...
- [ ] 3.11 Update only current-context completed task checkboxes and commit them with the matching implementation and BDD evidence.

...
- [ ] 3.1 Run `bun run openspec:vision -- commit-check implement-ext-lynx-macos-extension --phase apply` before product-code work starts and commit ready OpenSpec artifacts.
- [ ] 3.2 Add the official Lynx facade package `packages/ext-lynx` with typed commands, events, tests, README, and example path.
- [ ] 3.3 Add darwin platform package atoms `packages/ext-lynx-darwin-arm64` and `packages/ext-lynx-darwin-x64` with correct package metadata and publish file topology.
- [ ] 3.4 Add `crates/opentray-ext-lynx` as a `cdylib` and port the proven macOS runtime extraction / external bundle staging / launch lifecycle from the research CLI into the extension-owned runtime.
- [ ] 3.5 Keep Lynx runtime ownership outside `opentray-core` and `opentray-bin`; only generic dynamic extension discovery/loading may change.
- [ ] 3.6 Add a user-visible Lynx smoke path in the CLI, including explicit bundle-path input and clear output for launch success/failure.
- [ ] 3.7 Extend native artifact staging scripts, tests, and release workflow so darwin release jobs stage `libopentray_ext_lynx.dylib` plus `LynxExplorer.app.zip` into Lynx platform packages.
- [ ] 3.8 Add concise intent comments only at critical effect points where the runtime sidecar/package law would otherwise be easy to misread.
- [ ] 3.9 Add a changeset for any package releases required by the third-stage Lynx landing.
...
- [ ] 3.11 Update only current-context completed task checkboxes and commit them with the matching implementation / BDD evidence.

## 4. Verification

- [ ] 4.1 Run targeted Rust and TypeScript tests for the Lynx extension, release artifact topology, and CLI smoke surface.
- [ ] 4.2 Build or reuse a real `.lynx.bundle` and prove the local Lynx smoke path launches a visible macOS runtime through the generic extension path.
- [ ] 4.3 Run `bun run openspec:vision -- validate implement-ext-lynx-macos-extension` for this change.
- [ ] 4.4 Run repo-level verification after the Lynx-specific gates are green.

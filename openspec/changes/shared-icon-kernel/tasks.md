...
- [ ] 2.2 icon-kernel「Glyph defaults SHALL follow the continuous-curvature squircle standard」——glyph SVG 快照（figma-squircle path 参数 = 品牌路径 tile 参数）、1024 全幅 + 824-in-1024 变体几何断言、ICNS 10-tag 集合 + pHYs 72dpi chunk 存在、字体阶梯（拉丁内嵌 + CJK fontBuffers 真渲染 + 无静默丢字断言）。
- [ ] 2.3 icon-kernel「Kernel outputs SHALL carry full cache identity」——实现/recipe 变更触发再生成、未变更跳过（mtime/探针断言）。
...
- [ ] 3.1 `packages/icon` 脚手架：package.json（jsquash 系 + resvg-wasm + figma-squircle + icon-encoder + exifr 全部 `dependencies`；tsdown 构建；wasm/字体资产发布）、tsconfig、vitest；README 公开 API 契约先行。
- [ ] 3.2 `wasm.ts`：Node/Bun wasm 预加载器（readFile → `init(bytes)`；`@jsquash/png/decode.js` 子路径 init、`initResize`、resvg `initWasm`），失败抛可诊断错误（消费方决定回落）。
- [ ] 3.3 `glyph.ts`：`buildGlyphIconSvg`（figma-squircle 连续曲率 tile + 首字母 + accent）与字体阶梯（内嵌 OFL 默认字体资产 → OS 目录扫描 fontBuffers → 覆盖失败中性兜底；`loadSystemFonts` 恒 false）。
- [ ] 3.4 `raster.ts`：解码分派（png/jpeg/webp/avif + svg via resvg）、`resizeImage`（lanczos3 / 像素画点采样）、EXIF 方向（exifr）。
- [ ] 3.5 `compose.ts`：icon-compose 语义移植（foregroundStats/autoBackground 手写循环保持、squircle dest-in、824 变体、背景模板 PNG 资产迁移）。
- [ ] 3.6 `encode.ts`：pHYs 72dpi chunk 注入（PNG chunk 手术 + CRC32）、ICNS 10-tag（自 1024/824 双变体取源）、ICO 7 档、Linux PNG。
- [ ] 3.7 `generate.ts`：`generateOpenTrayAppIcon`（保留 `composed`/`macosSourcePath`；cache schema bump：jsquash/resvg 实现身份替换 sharp 版本；recipe bump）。
- [ ] 3.8 `default-icon.ts`：`generateDefaultAppIcon({ appName, accent?, outputDir, cacheDir })`（glyph SVG → 双变体 → 三平台目录 + manifest；缓存身份 = appName+实现+recipe）。

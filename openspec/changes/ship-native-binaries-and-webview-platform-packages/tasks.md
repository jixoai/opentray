...
- [ ] 2.6 Scenario: Given a dynamic extension library is resolved When the daemon validates it Then missing ABI version/init/command/deinit symbols produce structured load errors.
- [ ] 2.7 Scenario: Given a dynamic extension command is dispatched When it crosses the ABI Then Rust kernel types do not cross the dynamic library boundary.
- [ ] 2.8 Scenario: Given WebView platform package is installed When `load-ext webview` runs Then daemon locates the package-adjacent dynamic library through the generic extension host.
- [ ] 2.9 Scenario: Given WebView dynamic runtime is loaded When demo commands run Then `show`, `postMessage`, and `evaluate` remain visually observable.
- [ ] 2.10 Scenario: Given platform WebView cannot create a window When `show` runs Then the extension returns typed unsupported/capability failure instead of fake success.
...
- [ ] 4.9 Implement dynamic extension ABI structs and symbol validation in Rust without passing Rust kernel types across ABI.
- [ ] 4.10 Implement generic dynamic extension discovery from package-adjacent artifacts, user config, and `OPENTRAY_EXT_PATH`.
- [ ] 4.11 Move current WebView native runtime behind the dynamic extension boundary for supported platforms.
- [ ] 4.12 Ensure current internal WebView adapter is removed or demoted to explicit development-only fallback that still exercises the host contract.
- [ ] 4.13 Update CI release workflow to build daemon binaries and WebView dynamic libraries in a platform matrix.
- [ ] 4.14 Update publish job to download artifacts, stage them into package dirs, run pack validation, and publish with trusted publishing.
- [ ] 4.15 Configure changesets so daemon platform packages version with `opentray`, and WebView platform packages version with `@opentray/ext-webview`.
- [ ] 4.16 Add first-stage release changeset covering daemon binaries, WebView platform packages, and npm-installed smoke.
- [ ] 4.17 Run package bootstrap/trusted-publish check for all new WebView platform packages.
- [ ] 4.18 Add concise intent comments at artifact staging, resolver fallback, and ABI validation boundaries.
...
- [ ] 6.3 Run `bun run openspec:vision -- commit-check ship-native-binaries-and-webview-platform-packages --phase self-review`.

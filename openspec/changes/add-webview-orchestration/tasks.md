...
- [ ] 3.3b Broker 异步扩展事件宿主上下文（3.3 摩擦 #4 升格，D19 依赖）：opentray-bin 以真实 ExtensionHostContext 替换 `UnsupportedExtensionHostContext`（main.rs:733/756）——`send_event` → 按 extension 归属路由到客户端会话的 ExtEvent 帧（核心已有 RoutedEvent 路由基建，broker.rs 测试上下文即参照）；generic 扩展 ABI，无产品特判；完成后 ext-webview 的 per-view 事件从「随命令响应 flush」升级为回调即推。

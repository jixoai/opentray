...
- [ ] 3.4 Rust 布局：layer 数组 → 每层 Taffy 树求解（输入校验前置、逻辑→物理像素经 scale factor）→ setFrame 应用；resize 原生重算（windowDidResize 接线）；box 视图（layer-backed NSView，background/border/cornerRadius，不接受 first responder 实现输入穿透）；**布局提交事务含 overlay 安全区投影重算 + geometryChange 推送 + 拖拽区重注册（D23，单 webview 退化路径先行）**。**体积门禁（Owner 2026-09-11）**：dylib release 增量 ≤100 KB 继续 Taffy；100–200 KB 停下报数待裁决；>200 KB 弃 Taffy 转自研 flex 子集求解器（算法实现，Codex 复核后收口）。

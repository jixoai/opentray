...
| ---- | ------- | ---------------- | ---------------- |
| 1 | Issue #003 | `TrayOptions.icon` is required, but there is no helper to create one; only `rgba` actually works today. | The public tray icon path is too low-level for ordinary users and needs backend-side normalization. |
| 2 | Issue #003 | The smoke path already has a `createVisibleIcon()` helper, but it lives inside example/smoke code. | The visible icon recipe should stay as an internal smoke utility instead of becoming the ordinary app recipe. |
...
| 4 | User | “那你是觉得需要用户手动接触 helper 吗？还是改进 TrayOptions.icon 的参数类型，提升易用性，底层由 helper 处理复杂性？” | The public contract should get easier to use; backend complexity should stay behind the runtime boundary. |
...
| `packages/cli/src/client.ts` | `SpaceHandle.createTray()` forwards the provided tray options directly into the broker request. | The SDK stays a thin pass-through and does not own icon normalization. |
...
| `packages/cli/README.md` | The README now explains that native tray icon normalization happens in the backend and that `file` / `encoded` are accepted. | The docs should teach the ergonomic path first while keeping the backend boundary honest. |
| `crates/opentray-backend-tray-icon/src/projection.rs` | The backend projection layer now normalizes `encoded` and `file` tray icon sources into RGBA and reports typed decode failures. | This is the real normalization atom; it keeps file policy behind the native backend boundary. |
| `crates/opentray-backend-tray-icon/src/native.rs` | The native tray runtime now only materializes pre-normalized RGBA tray icons. | Native runtime should stay honest; the projection layer must own decode and file policy. |
| `openspec/specs/backend-adapters/spec.md` | The backend law now says the native tray-icon backend normalizes supported icon sources before tray materialization. | This change upgrades the backend law without inventing a public helper package. |
...
| ------------- | ----------------------- | ----------------------- |
| `openspec/specs/backend-adapters/spec.md` | The native tray backend now truthfully supports `rgba` plus backend-normalized `encoded` / `file` inputs. | Reuse as the hard backend law; keep the decode policy inside the backend atom. |
| `openspec/specs/client-sdk/spec.md` | `opentray` is the public consumer entrypoint and already owns tray creation convenience. | Keep the client SDK path thin and let the backend adapter own icon normalization. |
...
| ---- | ------------------- | ------------------------ |
| `packages/cli/src/smoke/daemon-tray.ts` | What the current human-visible tray path does for icon creation and tray launch. | Keep the visible icon helper as an internal smoke recipe. |
| `packages/cli/src/smoke/daemon-lynx.ts` | Whether the Lynx smoke flow also depends on local visible icon construction. | Keep the duplicated visible icon helper confined to smoke/examples. |
...
| Should icon decoding happen in the public SDK boundary or in a new package? | This decides whether ordinary users need to learn another helper package. | Keep the conversion in the existing backend adapter boundary; do not introduce `ext-icon-helper`. |
...
The product pressure is not “add another icon API.” The pressure is that the public tray path still leaks a low-level wire shape into everyday app code. The SDK should stay thin and let the backend absorb the icon conversion concern. The backend law stays honest and narrow: native tray rendering still only trusts RGBA after normalization.

### Final Visible Effect

A developer can create a tray with an ergonomic icon source and see a visible tray icon without writing their own pixel buffer code. The smoke/demo paths keep a shared internal visible-icon helper, but ordinary app code can point at a PNG file or encoded PNG bytes and let the backend normalize it. If the icon source is missing or cannot be decoded, the backend rejects with a typed, actionable error instead of silently pretending the tray icon was handled.

## Platform Diagnosis

- Current platform laws: the public `opentray` path already forwards tray icon shapes, the native tray backend owns tray materialization, and smoke/example code keeps a shared visible icon helper.
- Does this fit as a regular atom: yes. This is a backend adapter atom, not a client-side normalization helper.
- Does this require law upgrade: yes, but only in the backend adapter law. The client-facing tray contract can stay stable while the native tray adapter learns to normalize supported icon sources.
- Breaking update stance: prefer additive public ergonomics. Do not create a new public helper package. Do not add JS-side PNG decoding.
...
The developer writes the normal `createTray()` call and supplies an icon source that makes sense for a desktop app, such as a file-backed PNG. The tray appears with a visible icon. The developer does not have to hand-author RGBA pixels for ordinary app branding. The human-visible smoke/demo still proves the icon is nonblank, but the source of truth becomes the backend projection boundary rather than copy-pasted helper code.

### Interface Shape

- Public tray creation remains on the ordinary `opentray` path.
- `TrayOptions.icon` remains a required visible-tray input.
- The SDK boundary accepts ergonomic icon sources and forwards them unchanged to the broker request.
- The backend projection layer normalizes supported icon sources into the native-safe `rgba` asset shape before the native tray runtime sees them.
- The backend-facing native tray materialization still speaks `rgba` as the only runtime icon shape.
...
- Normalized backend shape: `Icon::Rgba` / `Icon["rgba"]` inside the tray backend before native tray materialization.
- Failure categories: missing source, decode failure, or unsupported input source that cannot be normalized.
- The smoke/demo helper stays as a reusable internal asset source rather than a user-facing requirement.

### Architecture Shape

- `packages/cli` stays a thin transport facade and does not own icon normalization.
- `@opentray/spec` keeps the wire/protocol icon law honest and unchanged.
- `opentray-backend-tray-icon` owns file/encoded PNG decoding, validation, and normalization to RGBA before native tray materialization.
...
| ---- | ---------------------------- | -------------------------- |
| none | The user already chose the direction: improve the tray icon input ergonomics instead of making ordinary users touch a helper package. | Keep the conversion boundary inside the backend adapter atom. |
...
| Push file/encoded decoding into the client SDK | That would add a JS image dependency and move asset conversion away from the backend atom. |
...
- Custom exit condition from intent: ordinary tray creation can accept an ergonomic icon source without manual RGBA construction, the backend normalizes supported icon sources to `rgba` before native tray materialization, and the docs/examples no longer teach a raw-pixel-only path as the ordinary one.

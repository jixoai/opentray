...
- [x] 2.1 icon-kernel「One shared icon kernel SHALL own app-icon generation」——包存在性/命名法（`packages/icon` → `@opentray/icon`）、三方消费无重复实现、无 sharp/libvips 依赖（`npm pack --dry-run` + 依赖树断言）。
...
- [x] 2.5 create-materialize-pipeline「Create icon pipeline SHALL consume the shared icon kernel」——create 产物走共享内核（wizard glyph 兜底 = runtime 默认同标准）、compose 语义保持（round-12 亮度/覆盖/前景像素保持回归 fixture）。
...
- [x] 3.10 vite-plugin 薄壳化：`openTrayAppIconPlugin`/`generateOpenTrayAppIcon` API 不变，内部 re-export `@opentray/icon`；移除 sharp/figma-squircle/icon-encoder 直接依赖。
- [x] 3.11 create 迁移：core 删除 icon-compose.ts/glyph 自有实现，改 import 共享内核；`writeGlyphIconTemp` 路径统一走 `generateDefaultAppIcon`；审计并迁移 tray 128px 等其余 sharp 用点，create 零 sharp。
- [x] 3.12 关键效果点意图注释（指向 plan.md D1–D11）。

## 4. Verification

- [x] 4.1 `@opentray/icon` vitest 全绿（SVG 快照/ICNS 结构/pHYs/像素容差/缓存命中/字体阶梯）+ 全仓 typecheck + 受影响包既有测试全绿。
- [x] 4.2 真实端到端取证（临时 HOME/OPENTRAY home）：省略 appIcon 启动示例 → bundle `CFBundleIconFile` + `AppIcon.icns` 合法（`iconutil` 往返）→ 二次启动缓存命中 → Dock 图标实际变化（截图/程序化取证）。
- [x] 4.3 回归：显式 appIcon 消费路径不变；vite-plugin golden 产物（recipe bump 后）合法；create wizard 预览与产物一致。
- [x] 4.4 发布审计：`npm pack --dry-run` 依赖清单只含 registry 真实包；grep dist 无私有 workspace import 残留（tsdown 依赖分类法）。
- [x] 4.5 fast-remix 复核（super-thinker 子代理）多轮闭环 + `validate`/`check` + 分 phase 提交。

## 5. Docs & Law

- [x] 5.1 AGENTS.md App Icon Law 修订（omitted → glyph 物化；`@opentray/icon` 所有权法；vite-plugin 薄壳表述）。
- [x] 5.2 skills/opentray 公开文档默认图标行为；根 README 简述；vite-plugin/create/icon 包 README。

## 6. Review Loop (fast-remix)

- [x] 6.1 super-thinker 复核（评分 7/10）发现两个阻塞缺陷并全部修复验证：B1 exifr CJS/ESM 互操作（真实 Node JPEG 解码冒烟通过 + 回归测试）；B2 CRC32 缺 `&0xff` 掩码（zlib.crc32 独立权威全 chunk 校验通过 + 回归测试）。非阻塞建议 1/2/6/8/9 已修（glyph 兜底统一 fileStem、缓存含 macOS 变体、死代码、诊断 mkdir、多 pHYs 全移除），3/4/5 以注释明示语义偏移。
- [x] 6.2 rebase 对齐最新 main（add-create-url-apps 四提交）：语义合并验证（URL 抓取与图标栈共存），全量 565 测试 + 6 包 typecheck 绿。

...
- Round: 1
- Status: discussion / research-plan, app identity derivation under review
...
| `不打算做任何向下兼容` | v0.9 may delete deprecated APIs, protocol fields, aliases, and docs in one break. | No migration shim required unless chosen for implementation staging only. |
...
| Should the display candidate field be called `icons`, `display`, or another name? | `icons` is user-suggested but includes text-only candidates; naming must not lie about ontology. | Prefer a neutral projection word, but record the user's `icons` map proposal as active. |
| Should `title` stay as a top-level sugar field? | If `icons`/display candidates are required for all display, top-level `title` may duplicate projection state. | Keep simple sugar if it compiles to the candidate map; no separate durable source. |
...
- Breaking update stance: full break for v0.9; no downward compatibility required.
...
Candidate responsive projection map from the user:

```ts
await createTray({
  id: "com.example.build",
  icons: {
    "icon-text": { icon, title: "Build" },
    icon: { icon },
    text: { title: "Build" },
  },
  menu,
...
Current concern: `icons.text` is semantically odd because `text` is not an icon. This plan keeps the user's map shape as an active proposal, but does not lock the field name until the identity and projection model are settled.
...
- display candidates: caller-authored projection options;
...
- remove or replace protocol `create-space`, `default-space`, `spaceId`;
...
| Display selection priority | Map keys need deterministic platform selection order. | Prefer platform capability order: `icon-text`, `icon`, `text`, unless user wants explicit priority. |
...
- [ ] 2. Continue product-law discussion until identity, app metadata derivation, and display fields are settled.
- [ ] 3. Write specs from settled v0.9 law.
...
| Should tooltip survive as a first-class field? | User questions whether `title` and `tooltip` overlap. | Tooltip is hover/accessibility projection; it can default from title. |
| Should display candidates live under `icons`, `display`, `presentation`, or another name? | Field name should match ontology. | Do not lock yet. |

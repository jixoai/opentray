...
- Round: 1
- Status: discussion / research-plan, icon projection type settled as `icon`
...
The "magic" is intentional: the same `icon` field can be a plain `IconImage`, a responsive candidate map, or both. `icon-only` and `text-only` are explicit only-mode candidates. The generic `IconImage` fields on the same object are fallback material, not a separate `icons` field.
...
- `icon` source: caller-authored image/text projection candidates plus fallback image facts;

...
- Round: 1
- Status: discussion / research-plan, direct tray vs managed broker-worker modes under review
...
Open a v0.9 design change where OpenTray drops `Space` as a user-facing and preferably internal concept, defines tray-first direct APIs, isolates each host program's broker/tray runtime, designs responsive tray display candidates, and separates direct tray capability from managed background broker-worker lifecycle without preserving backward compatibility.
...
new pressure:
caller/app identity -> runtime mode -> Tray atom -> responsive projection candidates
```
...
The latest design split refines that rule: a dead launcher is not a problem if it only launched a managed worker that remains the action source. The action source must be one of:

```text
direct mode:
developer-owned process -> createTray -> event handlers in same process

managed mode:
launcher -> createBroker/createWorker -> independent JS/TS worker entry
                                   -> create/register trays
                                   -> owns event handlers
```

The broker process alone is never the business logic owner. In managed mode, the JS/TS worker entry is the owner.
...
Managed-mode developer can provide a JS/TS worker entry that continues to own tray logic after the launcher exits:

```ts
// tray.worker.ts
import { createTray } from "opentray/worker";
...
- Current platform laws: archived current law already collapsed shared surface and pins one broker to one caller, but public SDK/protocol/docs still expose `Space`, `spaceId`, `createSpace`, `resolveDefaultSpace`, and surface-derived backend vocabulary. The new discussion adds a second axis: direct process-owned tray vs managed worker-owned tray.
...
```text
I have a long-lived tray app.
I give OpenTray a worker entry.
OpenTray starts the worker with Node/Deno/Bun.
The worker owns tray registration and events.
My launcher can exit without orphaning events.
```
...
Candidate managed mode:

```ts
const broker = await createBroker({
  app: { id: "com.example.build", name: "Build Monitor" },
  worker: {
    runtime: "node",
    entry: "./tray.worker.ts",
  },
});
...
Naming is not settled. `createBorker` in the user turn is treated as `createBroker` unless the typo becomes intentional product naming.
...
- runtime mode: direct process-owned mode or managed worker-owned mode;
- worker entry identity: in managed mode, the JS/TS file that owns business logic and event handlers;
...
direct mode:
  app process
      owns tray logic
      owns event handlers
      closes tray when process exits

managed mode:
  launcher process
      starts/stops broker-worker
  worker process
      owns tray logic
      owns event handlers
      can survive launcher
  native broker/runtime
      owns desktop/native projection only
```
...
- add an explicit managed worker lifecycle if the project chooses `createBroker`/service-worker-like mode;
- decide package split for direct tray API vs managed broker-worker runtime;
...
launcher exits
    |
    v
worker entry still alive?
    | yes                         | no
    v                             v
events route to worker       remove trays/routes
visible tray may remain      no orphaned effects
```
...
| Runtime mode split | Direct tray and managed worker solve different lifecycle problems. | Keep `Tray` pure; introduce managed runtime as a separate atom. |
| Package boundary | Background service is common but should not pollute tray ontology. | Prefer split package or explicit subpath boundary. |
...
| Should caller disconnect make the broker exit immediately or idle briefly? | Immediate exit is simpler; idle can make reconnect cheaper but must not keep visible effects alive. | Allow idle process only after all visible state and routes are removed. |
| Should `createBroker` live in `opentray` or `@opentray/broker`? | Determines whether background lifecycle is a first-class package or a convenience surface. | Prefer `@opentray/broker` plus optional top-level re-export only after boundaries are proven. |
| Should the worker entry import from `opentray/worker` instead of `opentray`? | Prevents worker-only APIs from leaking into direct mode. | Prefer explicit subpath. |
| Which runtimes are supported for managed worker entry? | User explicitly named Node/Deno/Bun. | Node first if implementation cost matters; spec should design for all three if v0.9 scope allows. |

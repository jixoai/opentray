...
- Round: 10
- Status: production native-host recovery remains approved; native client paint is added only as a separate source diagnostic experiment, with visual acceptance pending.
- Previous plan backup: `plans/plan-v11.md`
...
| User acceptance on 2026-07-16 | `Shell no activate -> Host +1px` cleared residue and restored the normal window appearance. | The production recovery can use the non-activating shell variant followed by a reversible host-only pulse; suppress WebView synchronization during the pulse to remove page jitter. |
...
- Does this require law upgrade: yes. User evidence accepts the non-activating shell transition plus native-only host pulse for the current normal-window residue class; source-host and final Mica/Acrylic visual verification remain required.
- Breaking update stance: no public API or migration. Debug diagnostics are opt-in and source-example scoped.
- User confirmations still required: final Mica/Acrylic visual confirmation that the production path clears residue without WebView jitter or input/focus regression.
...
- The default-off Native client paint switch fills the full HWND client rect black whenever `WM_ERASEBKGND` arrives and in `WM_PAINT`; it bypasses Softbuffer/WebView refresh so it remains independently attributable.
...
- `production recovery`: non-activating shell transition followed by a reversible native HWND width pulse inside one synchronous attached-surface-refresh suppression scope; it never synchronizes WebView geometry during either stage.
- `native client paint`: diagnostic-only per-window boolean; when on, the HWND fills its whole client surface black in paint and in any delivered background-erase message, with no WebView/Softbuffer follow-up.
...
Forbidden: raw standalone probe evidence as a product conclusion, `opentray-core` WebView branches, recurring resize timers, WebView synchronization inside the production host pulse, and promotion of unrelated candidates without new evidence.

### User Confirmation Gates

| Gate | Why confirmation is required | Default until user answers |
| ---- | ---------------------------- | -------------------------- |
| Final Mica/Acrylic acceptance | The accepted recovery changes shell behavior and must avoid page jitter, focus loss, and input regression. | Keep the production sequence scoped to normal windows and preserve diagnostics for further candidates. |
...
| Does full black `WM_ERASEBKGND` / `WM_PAINT` filling clear the real OpenTray material residue without masking WebView content or harming input? | The native-only probe is suggestive but has no WebView child. | Keep it diagnostic-only until the user records Mica/Acrylic evidence in `example:win32-bug`. |
...
| Promote native client paint to production from the standalone probe | The real OpenTray host still needs Mica/Acrylic, input, and page-content acceptance. |

...
- Round: 11
- Status: production native host painting is implemented and source-host verified; final human Mica/Acrylic/frameless/resize/reveal acceptance remains pending.
- Previous plan backup: `plans/plan-v12.md`
...
| 2026-07-16 | User | The visible residue is native-window residue, not WebView page pixels; only `Shell reset -> Host +1px` and `Shell reset -> Host +1px -> Host -1px` clear it, while the reverse order does not. | Model the artifact as a native host/DWM material-surface candidate; isolate restore activation mode without coupling it to geometry or WebView repaint. |
| 2026-07-16 | User | `Shell no activate -> Host +1px` clears residue; promote the pulse so it modifies only the native window and does not make WebView content jitter. | Replace the production shell-only clear with a non-activating shell transition plus one reversible native-only host-width pulse. |
| 2026-07-16 | User | The native-only material probe shows a new hypothesis: the HWND reports `WM_ERASEBKGND` handled but does not paint its full client surface; black `WM_ERASEBKGND` / `WM_PAINT` filling is visibly effective in the probe. Add it as a switch to `example:win32-bug`. | First add one opt-in native client-paint variable; the later user matrix promotes it to production. |
| 2026-07-16 | User | In the controlled matrix, gray/no-host-paint leaves native residue and resize exposes staircase retention; `R` only accumulates paint messages; framed and frameless are equivalent; Acrylic remains visible without residue; the black host mode is visually best. Reducing WebView to one third of the window still leaves residue in the uncovered native area. | Promote the complete native host painter to production for material backgrounds. Treat WebView/page bounds as a downstream child commit, not as the residue owner. |
...
| User matrix on 2026-07-16 | Controller/child/parent/host/present/invalidate/update/DWM-flush did not clear native residue. Shell reset alone and host geometry alone did not clear it; only activating shell reset followed by raw `+1px` or `+1px -> -1px` did, and reverse order failed. | Controller/page repaint is not the active repair axis. The next bounded candidate changes only the restore activation mode, with raw geometry remaining manually sequenced. |
| Intermediate user acceptance on 2026-07-16 | `Shell no activate -> Host +1px` cleared residue and restored the normal window appearance. | This bounded candidate proved the residue was native-host-owned; the later no-WebView black-paint matrix supersedes the shell/geometry sequence for production. |
| Production source-host smoke on 2026-07-16 | In the independently built DLL, `manual-clear` completed without any intervening `wm-size-controller-bounds` record; the later diagnostic-only shell commands still produced their own controller-bounds records. | The production suppression covers the shell transition and native pulse without changing the diagnostic atom semantics. This proves host routing, not Mica/Acrylic pixels. |
| User tray report on 2026-07-16 | The composition example's tray left and right interactions did not respond when launched with an explicit extension DLL. | The explicit-DLL source path skipped local broker preparation and could pair current extension code with the stale packaged broker. Preserve caller-selected extension loading while selecting the matching source broker unless its path is explicitly overridden too. |
| Explicit-DLL source smoke on 2026-07-16 | The composition example reported `broker binary: E:\dev\github\opentray\target\debug\opentray.exe` while loading the independently built extension DLL. | The launcher now keeps extension and broker on the same source revision; real left/right tray interaction remains human acceptance. |
| User regression report on 2026-07-16 | From the previous build, `Shell reset -> Host +1px` no longer clears residue. | Production all-refresh suppression was accidentally shared with the diagnostic raw-host atom, suppressing `WM_PAINT` that the accepted experiment still needs. Split the two policies. |
| `C:/Users/gaube/AppData/Local/Temp/native-material-host-paint-probe-20260716/src/main.rs` and its probe log | The standalone HWND has no WebView2; its experimental black mode fills the complete client area in both `WM_ERASEBKGND` and `WM_PAINT`, while the OpenTray-like mode returns handled without filling. The user reports the black mode removes residue. | Add the same full-client GDI path as one guarded variable in the real OpenTray HWND. The code/log prove message-path design, while visual repair remains user acceptance. |
| First OpenTray native-client-paint source smoke on 2026-07-16 | `InvalidateRect -> UpdateWindow` produced `wm-paint-filled` but not `wm-erasebkgnd-filled`. | Replace that request with `RedrawWindow(RDW_ERASE | RDW_INVALIDATE | RDW_UPDATENOW)` and rerun before accepting the switch as a full erase-and-paint experiment. |
| Second OpenTray native-client-paint source smoke on 2026-07-16 | `RedrawWindow(RDW_ERASE | RDW_INVALIDATE | RDW_UPDATENOW)` again recorded real `wm-paint-filled` without a delivered erase message; no bounds records appeared inside the switch operation. | The real host proves complete `WM_PAINT` black filling and preserves an erase handler for any delivered erase. Do not claim the host class receives erase on this path; visual Mica/Acrylic acceptance remains required. |
| User native-host matrix on 2026-07-16 | No-host-paint retains native pixels across resize; background reset does not clear them; frame/frameless does not change the result; Acrylic remains visible and clean; black host paint is the best visual mode; child WebView coverage does not bound the artifact. | The production invariant is a persistent top-level HWND material base. A post-hoc WebView resize or shell pulse is not the primary repair. |
...
| `玻璃城堡` / `没有扫地的守门人` | The host claims background erase but leaves old client content; a black fill is a clean material base rather than a page repaint. | Test complete native HWND client painting before blaming WebView2. |
...
Create a Windows-only `example:win32-bug` that reproduces rendering residue in the real OpenTray WebView host. It must provide every control from WebView Control's first Window card, plus direct probes for `clearWhiteBlock`, frameless transition, a one-pixel resize pulse, and a default-on Material host paint switch that can be disabled only for reproduction. Frameless mode must expose a transparent, draggable diagnostic titlebar with optional self-drawn controls.

### Underlying Drive

The old repair changed shell state, but the operator could see it fail while a tiny geometry mutation succeeded. The native-only matrix now shows that the missing invariant is a complete parent HWND client paint, with WebView2 acting downstream of that surface.

### Final Visible Effect

An operator launches one tray-owned diagnostic window, selects opaque/Mica/Acrylic, toggles frameless, invokes `clearWhiteBlock`, runs a one-pixel resize pulse, and can disable or restore full native black client painting. In frameless mode, a transparent page titlebar starts native dragging and can reveal self-drawn minimize, maximize/restore, and close controls. The page and console show which action was requested; the Windows host emits opt-in snapshot/timing evidence for the same HWND. The user can truthfully compare whether the stale pixels clear, whether input/focus survives, and whether shell state remains stable.
...
- Does this require law upgrade: yes. User evidence accepts a persistent black native client base for material-backed HWNDs; source-host and final Mica/Acrylic visual verification remain required.
- Breaking update stance: no public API or migration. Debug diagnostics are opt-in and source-example scoped.
- User confirmations still required: final source-host Mica/Acrylic visual confirmation that the production host paint clears residue without hiding WebView content or regressing input/focus.
...
- The default-on Material host paint switch reflects production. Diagnostics may turn it off to reproduce no-host-paint residue, then re-enable it to synchronously repaint the parent without WebView refresh.
...
- `outcome`: elapsed time, native host-paint state, focus/input observation, and human residue result. Human result remains evidence external to automatic logs.
...
- `automatic cleanup`: a bridge-owned boolean controlling queued host-surface recommits. Its disabled state is the diagnostic baseline and cancels pending next-message or delayed-reveal work; manual `clearWhiteBlock` bypasses it.
...
- `native residue hypothesis`: stale host/DWM material/redirection content visible behind an otherwise current WebView child; it is an evidence label, not an asserted DWM internals root cause.
- `shell restore variant`: paired minimize plus either activating restore or non-activating show; it does not include geometry, WebView synchronization, host present, paint, or DWM flush.
- `production recovery`: synchronous native host-surface recommit only; material uses a black parent GDI base, while plain opaque/transparent uses Softbuffer. It has no shell, focus, visibility, or geometry mutation.
- `material host paint`: production-on per-window policy for material backgrounds; diagnostics can disable it only to reproduce the old failure. `WM_PAINT` never chains into WebView/Softbuffer follow-up after the black fill.
...
@opentray/ext-webview Windows host
        ...
        +-- accepted native-only clear sequence
        |
        v
Win32 HWND + Wry Windowed WebView2 + DWM material
```

Forbidden: `opentray-core` WebView branches, recurring resize timers, WebView synchronization before the parent host commit, shell/geometry fallback in production, and promotion of unrelated candidates without new evidence.

### User Confirmation Gates

| Gate | Why confirmation is required | Default until user answers |
| ---- | ---------------------------- | -------------------------- |
| Final Mica/Acrylic acceptance | The accepted recovery changes parent-surface ownership and must preserve visible material, page content, focus, and input. | Keep the diagnostic disable switch until the source-host matrix passes. |
...
- [ ] 7. Verify the accepted native-only recovery in the source host, then collect final Mica/Acrylic no-jitter evidence before archive.
...
| Does the production black parent base clear the real OpenTray material residue without masking WebView content or harming input? | The native-only matrix proves parent ownership; the final source-host build still needs human confirmation with a real WebView child. | Ship the source build for immediate Mica/Acrylic acceptance and keep the diagnostic disable control. |
...
| Keep shell reset plus width pulse as production fallback | It hides the missing parent-paint invariant, changes shell/geometry state, and is unnecessary once the HWND owns its client surface. |
...
- Custom exit condition from intent: the user confirms the source-built OpenTray material host keeps Mica/Acrylic visible, leaves no resize/frameless/reveal residue, and preserves page input.

...
- Round: 2
- Status: Tauri implementation research folded into the navigator API design
...
> ```
>
> 是不是意味着，我们只要将引擎切换成tauri就能获得这个能力，这个能力不在底层wry上提供？

> 你先要调查一下他是怎么实现的

> 和Tauri保持一致的封装，完善我们的设计
...
| 4 | User | Investigate how Tauri implements `invoke` / `listen` before deciding the OpenTray WebView bridge. | The design must be based on source evidence, not an assumption that Wry provides a complete bidirectional API. |
| 5 | User | Keep the OpenTray wrapper consistent with Tauri after the investigation. | The navigator object should expose a scoped `invoke` / `listen` style facade, with high-level window methods layered above it. |
...
| Tauri `packages/api/src/core.ts` and `crates/tauri/scripts/core.js` at `25a68359` | Tauri public `invoke` delegates to `window.__TAURI_INTERNALS__.invoke`; the injected runtime creates success/error callback ids and sends `{ cmd, callback, error, payload, options }` to internal IPC. | OpenTray should copy the layering: public facade -> private internals -> transport, instead of exposing Wry IPC. |
| Tauri `packages/api/src/event.ts` and `crates/tauri/src/event/*` at `25a68359` | Tauri `listen` registers the JS handler as a callback id, invokes `plugin:event|listen`, and returns an unlisten function. Native events call `runCallback(handlerId, eventData)`. | OpenTray `navigator.window.listen` should be a first-class async API, not only DOM-style `addEventListener`. |
| Tauri `crates/tauri/scripts/ipc-protocol.js` and `crates/tauri/src/ipc/protocol.rs` at `25a68359` | Tauri prefers custom-protocol `fetch` for request/response and falls back to Wry `window.ipc.postMessage`; fallback/event delivery uses host-to-JS callback execution. | Wry's postMessage is one-way JS-to-native; OpenTray must define an explicit callback/event response law instead of assuming a native `onMessage` exists. |
| Tauri `crates/tauri-runtime-wry/src/lib.rs` at `25a68359` and Wry 0.55 local source | Tauri adapts Wry `with_ipc_handler`; Wry itself exposes `window.ipc.postMessage` and `evaluate_script`, not Tauri-like `invoke` / `listen`. | Switching to Tauri would buy the full app runtime, not a small Wry primitive. OpenTray should remain extension-owned and implement only the needed bridge law. |
...
| `和Tauri保持一致的封装` | Use Tauri's facade layering and async semantics, not Tauri as the engine. | Provide scoped `invoke` / `listen` APIs on the navigator capability object, then implement ergonomic methods on top. |
...
Expose frameless WebView window controls to page JavaScript through `navigator.window` and `navigator.opentrayWindow`, using a Tauri-consistent facade: scoped async `invoke`, async `listen` / `once`, and ergonomic window methods layered above that protocol. The transport remains an isolated private IPC channel that does not touch `window.postMessage`. Optional global `window.close` / `window.resizeTo` overrides are installed only when explicitly enabled.
...
A WebView page can call `await navigator.window.invoke("resizeTo", { width: 480, height: 320 })`, `await navigator.window.resizeTo(480, 320)`, or `const unlisten = await navigator.window.listen("resized", handler)` without importing OpenTray facade code or knowing about the native ABI. The page does not receive or emit global `message` noise, and `navigator.opentrayWindow` remains available as the prefixed fallback. If a capability is unsupported, the promise rejects with a typed unsupported error instead of silently faking success.
...
- Does this require law upgrade: Yes inside the extension atom: page JS now needs a stable injected navigator capability, Tauri-style scoped `invoke` / `listen` semantics, callback-id response delivery, and a private native channel law.
...
3. Custom HTML chrome can call either `navigator.window.invoke("resizeTo", ...)` or the sugar method `navigator.window.resizeTo(...)`.
4. The navigator object creates callback ids, serializes a scoped invoke request, and sends it through private internals.
5. The native extension validates the request, checks capability support, applies the window operation, and resolves or rejects the matching callback id.
6. Optional events such as `stylechange`, `moved`, `resized`, or `closed` are subscribed with `listen` / `once`, then delivered through callback ids rather than global `message` events.
...
- Tauri-consistent protocol methods: `invoke`, `listen`, and `once`.
- Ergonomic methods layered over `invoke`: `close`, `move`, `moveTo`, `resize`, `resizeTo`, `getStyle`, `setStyle`, and `getCapabilities`.
- DOM-style compatibility methods layered over `listen`: `addEventListener` and `removeEventListener`.
...
- Internal callback table: generated numeric callback ids, an internal `runCallback(id, data)` equivalent, and unregister-once semantics for request responses.
- Invoke request: `{ namespace: "opentray.window", cmd, callback, error, payload, options }`.
- Invoke success callback payload: the serialized result for the requested command.
- Invoke error callback payload: `{ code, message, details? }`.
- Listen request: an invoke command equivalent to `{ cmd: "listen", payload: { event, handler } }`, where `handler` is a callback id.
- Unlisten request: an invoke command equivalent to `{ cmd: "unlisten", payload: { event, eventId } }`.
...
- The script installs private OpenTray internals analogous to Tauri internals, but scoped to `opentray.window` and not exposed as the promoted public API.
- The script uses Wry IPC internally where available, but does not expose `window.ipc` as OpenTray's public API.
- Request/response and event delivery are defined by callback ids, not by a raw native `onMessage` assumption.
- Host-to-JS callback execution is an implementation detail allowed only behind the private internals boundary; page authors see promises and `listen` callbacks.

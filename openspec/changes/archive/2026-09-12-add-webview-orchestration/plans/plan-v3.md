...
> 本版以 D18–D22 与各 spec delta 修订收口；R2 评分 6.2/10（+0.7），阻塞 R2-B1–B19
> 全部采纳——本版（plan-v3）以 D18–D23 + R2 增补（并入 D2/D6/D11/D12/D13/D18/D19/D20/D21）
> 收口；报告 `/tmp/codex-review-add-webview-orchestration{,-r2}.md`。
...
| D2 | **契约面**：`createWebview / destroyWebview / listWebviews` + `navigate / back / forward` + `urlChange / titleChange / focused` 推送事件 + per-webview `focus`（`WebviewHandle.focus()` 显式命令 + `focused` 事件，窗口级 focus 保持独立）+ **`webviewId` 只读属性**（`currentWebview()` 方法否决，改为桥上只读属性）。**`createWebview` 携带可选 per-child `bridge` 策略参数（R2-B4/B11）**：默认 `off`（无桥——任意第三方 content 天然无桥）；toolbar 等受信子视图显式开启（同一声明式策略面族：navigator window/screen、native API、page access）；per-child bridge 策略对该 webview 生命周期内 bootstrap-immutable（与既有 bootstrap 法同构）；进两平台 DTO | 缺导航与事件成员 toolbar 载不起；webviewId 是布局与通道的寻址基础；不定义 per-child 策略则实现者可能在「全部继承」与「全部无桥」之间随机选择，安全边界漂移 |
...
| D6 | **多 webview 与半透明样式互斥（单一错误码，R2-B2 收口）**：webview 跨层**不透明**重叠合法；但「半透明影响样式」（frameless、material、以及未来任何 per-view 透明背景）与多 webview 组合**不可达**——在 child 创建时、在窗口样式**变更时**（向已持有 >1 webview 的窗口应用 frameless/material 同样拒绝）、在 layout commit 时三个检查点全部报同一类型化错误 `multiwebview_unsupported_style`（拒绝发生在变更/提交前，旧状态保持）。原拟的独立 `translucent_overlap` 错误码删除——material 是窗口级事实，在第二 child 创建时已被拒，layout 分支永远不可达，两个错误码构成假分叉 | 单一规则单一代码可推理；把禁令精确化到真正未验证的部分（半透明组合）；补上样式变更检查点堵住「先建后改样式」的绕行 |
...
| D18 | **所有权三元组法（Codex B3）**：多 webview 状态按 `(appId, trayId, sessionId)` 归属；windowId 会话内唯一；webviewId 窗口会话内唯一；`session_closed` 必须按 session id 精确清扫（**封死现存 mod.rs:587-590 忽略 id 清全局的缺陷**）；lease 断开只清本 lease 的 N 个 webview；交叉清扫 BDD 用**同 app、distinct trays**（同 tray 第二 session 违反一 tray 一 active window session 法，在创建处即被拒，不进清扫分支）；协议帧携带 owner tuple 字段。生命周期 requirement 的**标题保持原文**（`...scoped to surface tray and lease`）——MODIFIED 匹配以标题为 merge identity，术语弃用发生在正文（R2 非阻塞项澄清） | AGENTS.md Extension cleanup law 的直接投影；生命周期 requirement 正文弃用「surface」措辞回归 App/Tray/Session 本体 |
| D19 | **事件传输闭环（Codex B8 + R2-B9/B10/B14 收口）**：`urlChange/titleChange/focused` 是 per-view 推送帧；**帧 schema 固定**：`{ owner: {appId, trayId, sessionId}, windowId, webviewId, kind, seq, payload }`——owner tuple 随帧（解决会话内 id 撞键），`seq` 为 per-view 单调递增序号，payload ∈ `{url}` / `{title}` / `{focused: boolean}`（focused 为获得/失去焦点边沿事件）；订阅 = facade 监听器（wire 层有显式 subscribe/unsubscribe 帧，codec fixture 固化）；**初始快照与竞态**：事件为纯推送不重放，当前值经 facade 查询命令 `getUrl()/getTitle()` 获得，查询返回 `(value, seq)`，消费者先订阅后查询并丢弃 `seq ≤ 查询 seq` 的事件；`focused` 无查询（边沿语义）；**新事件禁止复用 16ms drain 轮询**（原生回调直推事件通道）；传输语义进 2.1/2.4/3.5 三层测试 | CPU 轮询成本法；drain 是存量路径，不是新事件的观察机制；无 seq 的订阅竞态会让「地址栏真值」依赖时序 |
| D20 | **通道精确语义（Codex B5/B6 + R2-B3/B5/B6/B15/B16 收口）**：`listMessageChannels` = live + closed 墓碑（每会话 LRU 上限 32 条，destroyed 不再列出）；**close 与 destroy 是两个 API**——`close()`：优雅关端 → 通道 `closed(reason=explicit)` + 墓碑保留；`destroy()`（facade `destroyMessageChannel(id)` / 桥 `endpoint.destroy()`，任一参与者可调、幂等、重复调用 no-op）：立即移除状态 → 存活端点收到 `closed(reason=destroyed)` 事件 + 墓碑即刻消失；destroy 另两个入口 = 容量逐出 / 会话关闭。队列上限 = 每端口 ≤1000 条 **且** ≤1 MiB（按累计值计，边界值合法）；**字节计数规则**：字符串 payload 计原始 UTF-8 字节；JSON payload 计其 **canonical 紧凑序列化**的 UTF-8 字节（键按 UTF-8 字节序稳定排序、无空白、数字最短表示、禁止 NaN/Infinity/undefined/函数——非法值在 post 时以类型化错误 `invalid_payload` 拒绝），canonical encoder 纳入 codec round-trip fixture；单条 payload > 1 MiB → `payload_too_large`（不入队、不关通道）；超限累积 → 关通道 reason `queue_overflow`。**跨会话语义（R2-B3）**：良构页面输入构造不出跨会话目标（桥只暴露本会话 id——对页面是构造上不可表达）；broker 对每个创建按 owner tuple 校验，畸形/敌意输入以 `session_scope` 拒绝、零部分状态。类型化错误码注册表：`unknown_view / invalid_layout_measure / multiwebview_unsupported_style / bridge_required / session_scope / not_open / payload_too_large / invalid_payload`（reason 与 error 是两个命名空间） | 「bounded 但无数值」不可验收；list 自相矛盾必须裁决；close/destroy 不分则墓碑语义无法验收；非 canonical 编码让 TS/Rust 两端在同一值上得出不同边界；稳定错误码是协议契约 |

...
| D24 | **加载进度事件（Owner 走查反馈，2026-09-12：「加载进度没有提供给 toolbar」）**：D19 统一事件族增 kind `loadState`，payload `{ phase: "started" \| "finished" \| "failed", url, errorCode?, progress? }`（progress ∈ [0,1]，不可得时省略——Windows 无原生进度暴露，phase 驱动不确定态进度条；macOS 可接 WKWebView estimatedProgress 时填充）；原生导航回调直推（mac: wry on_page_load 已有 Started/Finished + WKNavigationDelegate 失败路径；win: NavigationStarting/Completed + CoreWebView2 NavigationFailed）；toolbar 页渲染顶部细进度条（started 起、finished/failed 收） | 地址栏「正在发生什么」是浏览器直觉的一部分；phase 语义跨平台可得，progress 尽力而为不撒谎 |
| D25 | **favicon 进 toolbar（Owner 走查反馈，2026-09-12：「favicon 没有同步给 toolbar」）**：v1 走 **toolbar 页客户端自取**——按当前 url 的 origin 直接 `<img src="{origin}/favicon.ico">`（图片渲染不受 CORS 限制；加载失败回落首字母 glyph）；**不经原生面**（content webview 无桥，页面 rel=icon 不可观测；WKWebView 无 favicon API；create 时抓取的 favicon 是应用身份不是页面导航真值）。原生 favicon 事件留作后续协议演进位（若未来需要 per-navigation 精确 favicon） | 浏览器地址栏 favicon 的 80% 价值由 origin/favicon.ico 覆盖；零原生成本；诚实分层（页面级真值归页面手段） |
| D26 | **新窗口打开族，默认开启（Owner 走查反馈，2026-09-12：「默认开启 `a[target]` 的支持：在新窗口中打开，点击鼠标中间也能做到，右键菜单也要有相关的支持」）**：新增**辅助弹出窗（auxiliary popup window）**能力——macOS 实现 `WKUIDelegate.createWebViewWithConfiguration`（target=_blank / window.open / 中键点击 / 右键「在新窗口打开链接」四路全部汇入此 delegate），Windows 实现 `NewWindowRequested`（Handled + 自建窗口）；弹出窗 = **朴素单 webview 新 OS 窗口**（v1 无工具栏——OAuth/外链场景主导；title 单向跟随文档；**不占 tray 的 window session**——辅助窗列表独立追踪，session/lease 关闭时全部回收，与 living spec「Lease cleanup closes owned popup」场景对齐）；可开多个；capability DTO 增 `popup_windows`；样式互斥/布局协议不适用于弹出窗（无 setLayout 面）。URL 应用与命令应用的 toolbar content 一并生效（载体无关，native 层能力） | Owner 明示默认开启；四条入口（target 属性/window.open/中键/右键）在两平台各自的原生 new-window 管线汇合，一个 delegate 各自覆盖；朴素窗 v1 是 OAuth 语义的安全默认（无地址栏不可被钓鱼导航，可后续升级带工具栏形态） |

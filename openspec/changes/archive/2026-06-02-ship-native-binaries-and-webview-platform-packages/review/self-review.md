...
This change is ready for archive. Post-review release operations recovered the initial publish path, and a fresh npm-registry install smoke in `/private/tmp/opentray-closeout-Neb73z` now proves `opentray daemon health` and `opentray smoke daemon-tray` work from installed packages. Registry/docs parity for the next public vocabulary release remains a broader release-state concern, not a blocker for archiving this packaging change.
...
| Full release readiness | Pass for this change scope | Missing WebView platform packages are published, trust-configured, and fresh npm install smoke now passes for daemon health and daemon tray smoke. |
...
- Next public release should publish the current Space/Session vocabulary and navigator-window bridge so npm latest matches repository docs.

...
| `packages/cli/src/smoke/daemon-tray.ts` | The human smoke now declares exactly one plain item, `Open WebView`, with `primaryEvent: true`. | It directly exercises the macOS single-primary case where clicking the status item should not open a menu. |
...
| `openspec/changes/enrich-webview-window-macos-capabilities/*` | WebView windows are opened through extension commands from menu events. | Reuse the existing WebView open command behind the single `Open WebView` primary item, without coupling tray law to ext-webview. |
...
The operator can run the daemon tray smoke, click the tray icon in the platform's primary-action case, and see the same event they would have received by selecting the marked menu item. The demo intentionally declares only one menu item so macOS can direct-trigger it without showing the menu. The existing `menuClick` event handler opens the WebView window; no app code needs a second `trayPrimaryClick` handler unless a later feature proves it necessary.
...
The acceptance demo uses the last macOS shape: its native menu declaration contains only `Open WebView`, marked `primaryEvent: true`, so a click on the status item should directly emit `menuClick` and open the WebView rather than first showing native menu chrome.

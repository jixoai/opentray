...
3. Restore native overlay, Chrome-PWA-like Windows caption-button colors, and measurable full-client geometry.
...
| Overlay button colors | `windowControlsOverlay` was a Boolean gate, so no requested color ever reached `AppWindowTitleBar`. | Keep `true` compatible and accept an explicit overlay options object whose colors become WinRT `IReference<Color>` values. |
| Overlay usable width | DWM caption-button bounds describe one button cluster and were reverse-engineered into a titlebar inset. | Read `AppWindowTitleBar.LeftInset`, `RightInset`, and `Height` on the HWND-owning STA. |
...
| Window drag animation | `WM_EXITSIZEMOVE` was treated as resize-only and triggered the legacy white-block `SW_SHOWMINNOACTIVE -> SW_RESTORE` shell-state reset after every drag. | Track one native size/move interaction; authorize the workaround only when `WM_SIZE` was observed before exit. |
...
create hidden HWND
  ...
  -> AppWindowTitleBar.SetButton*Color(IReference<Color>)
  -> fit WebView2 to client rect
  -> show / focus
```

Overlay colors and metrics use a fresh AppWindowTitleBar reference only on the HWND-owning STA.
No AppWindow object crosses apartments or survives the native call. `LeftInset`, `RightInset`,
and `Height` are the safe-area authority; DWM caption-button bounds are not a substitute.
...
### Native Size/Move Interaction

```text
WM_ENTERSIZEMOVE
  -> active; saw_resize = false

WM_SIZE during active interaction
  -> saw_resize = true; optional throttled white-block repair

WM_EXITSIZEMOVE
  -> saw_resize ? resize-only repair : no shell-state mutation
```

`WM_ENTERSIZEMOVE` and `WM_EXITSIZEMOVE` cover both dragging and resizing.
Pure movement remains a geometry interaction, not a window-state transition.
...
- Keep `windowControlsOverlay: true` compatible while adding
  `windowControlsOverlay: { backgroundColor?: "#RRGGBB", symbolColor?: "#RRGGBB" }`.
- Keep `TrayHandle.getBounds()` and `WebviewPlacementKit` public shapes unchanged.
...
| Rust tests | ext-webview `64`, tray backend `26`, vendored tray-icon `2` plus `5` doc tests. |
...
- Moving a translucent Windows WebView does not run the white-block shell-state reset.
- Windows overlay button colors visibly differ from system white/black defaults when configured.
- `getTitlebarAreaRect()` uses AppWindow insets and tracks the native caption-control boundary across resize and DPI changes.

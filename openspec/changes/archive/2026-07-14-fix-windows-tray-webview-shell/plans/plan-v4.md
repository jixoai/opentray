...
- Round: 4
- Status: Source-example Vite ownership and Windows-visible retry are complete; self-review is refreshed.
...
| Example Vite readiness | `bun run dev` owns a Bun wrapper plus a Node/Vite child. The wrapper can exit before buffered Vite output is consumed, and child Vite listeners can outlive it. | Spawn the actual Vite Node entrypoint and treat child-process close as the stream-flush boundary. |
...
example process
   ...
   +-- node vite/bin/vite.js
           |
           +-- Local URL -> route readiness -> WebView

example shutdown
   |
   +-- terminate actual Vite process -> no orphan listener
```
...
- Source WebView examples spawn the actual Vite process, not `bun run dev`.
...
| Source Vite ownership | `5173` reservation forced `http://localhost:5174/webview-control`; bridge smoke passed. A direct `startDevServer()` close left only the reserved `5173` listener, then no listeners after reservation cleanup. |
...
- Source `example:webview-control` owns the actual Vite listener and can consume its selected Local URL.

...
- Previous plan backup: plan-v1.md
...
| 7 | User | `ext-webview` 的 remote URL native capability 暴露属于安全边界，建议参考 CSP 的声明式思维来定义，不必做复杂语法解析，但要能细化控制某一项并便于未来扩展。 | `show(...)` 需要增加 source-scoped capability policy，而不是继续只靠零散布尔开关。 |
| 8 | User | 同意补 `keepOnTop`，并要求把 P0/P1/P2 一步做到位。 | window contract 需要增加常见的 top-most 行为，同时保持平台 unsupported 边界清晰。 |
| 9 | User | 先把 change 写好，再继续讨论 P3：当前高斯模糊/材质是否本来就是平台专属能力。 | 这轮 intent 需要同时记录“通用 contract”与“平台专属材质语义”的分层决策。 |
...
| `~/.cargo/registry/.../objc2-app-kit-0.2.2/src/generated/NSWindow.rs` | `NSWindow` 暴露 `setLevel`，并提供 `NSNormalWindowLevel` / `NSFloatingWindowLevel`。 | macOS `keepOnTop` 可以直接留在 ext-webview 自己的 AppKit window atom 里实现，不需要 Tao。 |
...
| “参考 CSP 的声明式规范来做定义” | 能力暴露要按 source/direction 细粒度声明，并具备 fallback 心智，而不是只堆布尔开关。 | The command contract should own a source-scoped capability policy. |
...
把 `ext-webview` 做成一个在 macOS 上更完整的 window atom：继续保留现有 `navigator.window` law，但补齐无边框、透明背景、材质层、title、icon、screen、keep-on-top 等能力，并通过 `show(...)` 的 declarative options 控制 page/native 双向同步、global binding 与 source-scoped capability policy。

### Underlying Drive

用户真正要验证的不是“能不能开一个 WebView 窗口”，而是 OpenTray 的 extension law 是否足够强，能在不引入 Tao、也不回退到 daemon special case 的前提下，让一个 extension 自己拥有完整的 window capability surface。只要这次还需要 core 特判、或者为了材质层重造 runtime foundation、或者让 remote URL 默认拿到 native bridge，这条 law 就没有站稳。
...
- 页面可以通过 `navigator.window` 控 title/icon/style/keepOnTop，也可以在创建时声明 `document.title` <-> native title、favicon <-> native icon 的同步策略。
- 页面可以通过 `navigator.screen` / `navigator.opentrayScreen` 获取标准心智下的 screen details，并可选把 `window.getScreenDetails()` 绑定到同一条能力路径。
- remote URL 页面默认不会拿到这些 bridge；只有 declarative capability policy 显式放行的 source 才会获得对应能力。
...
- User confirmations still required: none before implementation; final visual acceptance should确认透明/材质/title/icon/screen/keepOnTop 的真实体验是否满足用户心智。
...
2. 页面可通过 `navigator.window` 查询能力、设置 title/icon/style/keepOnTop，或监听 title/style 等变化。
...
6. 如果页面来源不满足 declarative capability policy，对应 bridge 不会被注入，也不会开启 page-to-native sync。
...
- `show(...)` 保持现有基础字段，并以 additive 方式扩展：
  ...
  - `title?: string`
  - `icon?: Icon`
  - `style?: { frameless?: boolean; transparent?: boolean; backgroundEffect?: string | null; keepOnTop?: boolean }`
  ...
  - `nativeApiPolicy?: { defaultSrc?: string[]; window?: string[]; screen?: string[]; windowGlobals?: string[]; screenGlobals?: string[]; titleSync?: string[]; iconSync?: string[] }`
...
- opt-in global bindings:
 ...
 - source-scoped native capability policy:
   - directive-first, fallback-to-`defaultSrc` mental model inspired by CSP
   - no need for full CSP parser; the important law is explicit per-capability allow-lists by source class/scheme
...
- Declarative sync state:
  ...
  - native capability policy directives resolved against the shown page source
...
- `crates/opentray-ext-webview`
  - owns parser changes, bootstrap script changes, title/icon/screen/state sync law, source-scoped capability policy, and macOS native projection
- `window-vibrancy`
...
- no remote URL native bridge enablement by accident; page-exposed native capability must be policy-approved
- no re-growing a monolithic macOS runtime file when the capability family is already large enough to justify module boundaries
...
| Platform-specific material naming | 用户正在追问高斯模糊/液态玻璃等是否应该被视为平台专属能力。 | 先保持通用 `backgroundEffect` contract + capability gating，再决定是否需要 substrate-specific namespace。 |
...
| 通用 `backgroundEffect` contract 是否足以承载未来 Windows/macOS 差异材质，还是需要 substrate-specific namespace？ | 影响 P3 是否现在就抽象平台私有 API 面。 | 先把材质视为 capability-gated effect family，再根据第二个平台实现的真实分叉决定是否拆出平台私有面。 |
...
| 继续让 remote URL 只靠 `nativeWindowApi` / `nativeScreenApi` 这些布尔开关拿到 bridge | 这是 capability boundary 漏洞；需要 source-scoped declarative policy。 |
| 在继续加能力的同时维持单个 `macos.rs` 巨石文件 | 这会把 runtime、policy、sync、screen、style 全部耦成一个维护边界。 |

## Exit Conditions

- Default max review iterations: 2
- Issue recurrence threshold: 2
- Custom exit condition from intent: `ext-webview` 在 macOS 上拥有完整且 extension-owned 的 window metadata/material/screen/top-most capability surface；`show(...)` 能声明 sync/binding policy 与 source-scoped capability policy；`navigator.window` 与 `navigator.screen` 同时可用；实现、测试、README 与 OpenSpec 一致；不存在 Tao 依赖倒灌、remote bridge 默认泄漏、或 daemon special case。

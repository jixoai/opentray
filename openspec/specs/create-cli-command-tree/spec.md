# create-cli-command-tree Specification
...
`create-opentray create [target-dir]` SHALL create a v1 application without opening a browser, launching a prompt, or implicitly running any command; the explicit `--open` flag MAY launch the materialized application after a successful apply (the wizard's Open App action as a flag — opt-in, never the default). It SHALL accept either a complete v1 input document or explicit options that compile into v1 desired state. Explicit options SHALL include app identity/name, application and tray icon sources, exact command argv/cwd/env OR a single `--url` application source, package manager, icon smoothing, developer mode, window/shell options, and relevant apply controls.

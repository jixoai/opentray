# dialog-extension Specification

## Purpose
TBD - created by archiving change add-ext-dialog. Update Purpose after archive.
...
While any dialog is open, the broker SHALL keep dispatching tray events for the same session's other trays and mounts, evidenced by transport/request timelines rather than window screenshots. On macOS the dialog SHALL run through a modal-session stepping state machine (`Created → Presented → Stepping → Dismissed | Revoked`) driven by a broker-owned scheduler: `poll_owner` returns only `Pending { next_deadline, wake_reason }` — poll never carries a terminal payload, every terminal is submitted solely through the DeferredCompletionPort — the broker owns one coalesced `DialogPollDue(generation)` user event, idles via `ControlFlow::WaitUntil(min deadline)`, and re-arms that wake whenever a native callback advances an atomic deadline; native AppKit callbacks may only write the atomic deadline (no broker pointers, no unbounded self-wakes, no direct winit calls); revocation removes the schedule before `endModalSession`; stale due events fail generation checks; and a bounded per-loop poll quota (at most 4 owners, one modal step each) prevents menu/transport starvation. The extension SHALL NOT name or hold any winit `UserEvent`/`EventLoopProxy`, and a bare `runModal` from command dispatch is prohibited. On win32 each active `(appId, trayId, sessionId)` SHALL own a bounded dedicated COM STA worker — never the winit owner loop thread — with a frozen numeric contract (worker cap 8, worker-limit rejection `dialog_worker_limit_reached` before any queuing, worker startup/entry timeout 3 s, join timeout 2 s, per-worker `WM_APP` close dispatcher) and a two-branch failure transaction: a worker that fails BEFORE entering the native modal call produces a synchronous typed error response on the original `requestId` (`dialog_presentation_failed`) with no operation, no `Accepted`, and no terminal frame; only a failure AFTER entry settles as `Accepted` followed by a terminal error payload. `Accepted` on win32 SHALL mean honestly that the worker entered the native modal call (TaskDialog presentation evidenced by the `TDN_CREATED` callback; `IFileDialog::Show` has no documented pre-return shown signal), never that a queued request was presented. A worker that has not exited SHALL never have its library deinit'd or dlclosed (join timeout defers cleanup until natural exit or abandons cleanup with a fatal diagnostic). UI-affine native instances SHALL live in an owner-thread registry; the blanket `ExtensionInstance: Send` / `unsafe impl Send` assumption SHALL be removed or proven never-moved, with only copyable request data and the host-owned thread-safe port shim crossing threads. At most one dialog SHALL be active per `(appId, trayId, sessionId)`; a second concurrent show for the same scope SHALL reject with typed `dialog_session_busy` before native presentation. The macOS owner-loop probe SHALL precede ordinary dialog implementation and SHALL cover wake starvation, exit races, and step/menu-frame interleaving.

#### Scenario: Pre-entry failure is a synchronous requestId error, never an orphan terminal

- **GIVEN** a win32 show command whose STA worker times out (3 s) before entering the native modal call
- **WHEN** the failure is reported
- **THEN** the client SHALL receive one synchronous typed error response carrying the original `requestId` (`dialog_presentation_failed`), no `ExtCommandAccepted` frame and no `ExtOperationTerminal` frame SHALL exist for the request, and the Node pending map SHALL settle it as an ordinary request error

#### Scenario: Post-entry failure settles as Accepted then terminal error

- **GIVEN** a win32 show command whose worker entered the native modal call and the call then fails
- **WHEN** the failure is reported
- **THEN** the wire timeline SHALL show `ExtCommandAccepted` first and exactly one `ExtOperationTerminal` whose payload is the `{ kind: "error" }` branch, and no second terminal SHALL be deliverable for that operationId

...
for (const kind of kinds) {
  ...
  if (kind === "runtime" && nativeTarget.runtimeCarrierArtifact !== undefined) {
    const templateSource = join(root, "packages/darwin-app-carrier/Info.plist");
    await stageArtifact(
      root,
      templateSource,
      nativeTarget.runtimeCarrierArtifact,
    );
    console.log(
      `copied Darwin app bundle template: ${templateSource} -> ${nativeTarget.runtimeCarrierArtifact}`,
    );
  }
}

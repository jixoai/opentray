...
export const evaluateSize = (bytes) => {
...
export const discoverEmbeddedPackageDirs = (root = ROOT) =>
...
const runRealPack = async (packageDir) => {
  ...
  try {
    ...
    const tarballPath = join(packDestination, entry.filename);
...
const runDryPack = async (packageDir) => {
  const stdout = await runNpm(["pack", "--dry-run", "--json"], { cwd: packageDir });
  const entries = JSON.parse(stdout);
  const entry = Array.isArray(entries) ? entries[0] : entries;
  if (entry === undefined) {
    throw new Error(`npm pack --dry-run produced no manifest for ${packageDir}`);
  }
...
export const measurePackage = async (
  packageDir,
  options = {},
  pack = options.dryRun ? runDryPack : runRealPack
) => {
...
export const formatReceiptLine = (receipt) => {
  const files =
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (dryRun) {
    console.log(
      "pack-size DRY-RUN WARNING: development early-warning only; NOT release evidence and NOT eligible for the packaging GO (add-ext-dialog §6.3)"
    );
  }

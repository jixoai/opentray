...
# 1. Bind the generic carrier builder to the exact OpenTray broker executable.
# 2. Declare release-default macOS privacy usage families.

set -euo pipefail

output_zip="${1:-${OPENTRAY_DARWIN_RUNTIME_CARRIER_ZIP_OUT:-}}"
broker_executable="${2:-${OPENTRAY_DARWIN_RUNTIME_BROKER_BIN:-}}"
if [[ -z "${output_zip}" || -z "${broker_executable}" ]]; then
  echo "usage: bash scripts/release/build-darwin-runtime-carrier.sh <output-zip-path> <broker-executable>" >&2
  exit 1
fi

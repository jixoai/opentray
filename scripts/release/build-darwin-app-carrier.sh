...
# 1. Materialize a macOS app bundle from one carrier source configuration or executable.
...
if [[ -z "${output_zip}" || -z "${source_dir}" || -z "${app_name}" || -z "${binary_name}" ]]; then
  echo "usage: bash scripts/release/build-darwin-app-carrier.sh <output-zip-path> <source-dir> <app-name> <binary-name> [executable-source]" >&2
  exit 1
fi
...
if [[ -z "${executable_source}" && ! -f "${source_dir}/main.swift" ]]; then
...
if [[ -n "${executable_source}" ]]; then
  case "${executable_source}" in
    /*) ;;
    *) executable_source="${caller_dir}/${executable_source}" ;;
  esac
  if [[ ! -f "${executable_source}" ]]; then
    echo "missing Darwin carrier executable: ${executable_source}" >&2
    exit 1
  fi
  cp "${executable_source}" "${build_dir}/${binary_name}"
else
  swiftc \
    -O \
    -framework AppKit \
...
set_plist_string "CFBundleExecutable" "${binary_name}"

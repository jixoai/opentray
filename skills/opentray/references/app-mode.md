<!--
Orthogonal intents (maintained 2026-09-15; original user requests: summarize the
skill-creator-v2 appMode adaptation as a public consumer tutorial, including repeatable
development-owner takeover, with decisions for different runtime scenarios; after a
downstream consumer hand-rolled a rounded-rectangle plus iconutil catalog that missed the
macOS standard, document where a platform-correct App identity icon comes from):
...
4. Route brand App identity icons through the shared @opentray/icon generation kernel.
5. Provide persistent diagnostics and an acceptance matrix.
-->
...
- [Generate A Platform-Standard App Icon](#generate-a-platform-standard-app-icon)
...
`appBundle.defaultAppIcon: false`) when you want different behavior. Generate
that explicit catalog with the `@opentray/icon` kernel — see
[Generate A Platform-Standard App Icon](#generate-a-platform-standard-app-icon).
...
const tray = (await createTray(
  ...
  },
  {
    ...
    // Platform-standard AppIcon array generated by the @opentray/icon
    // kernel (see "Generate A Platform-Standard App Icon" below); omit
    // it to use the runtime's appName glyph default.
    appIcon,
  },
)).extend(WebviewExt);
...
## Generate A Platform-Standard App Icon

`appIcon` is App identity — the Dock/taskbar artwork. It is not tray artwork
and not a page favicon. Never hand-roll the catalog: rounded-rectangle exports
and direct `iconutil`/`sharp` encodes miss the kernel's continuous-curvature
squircle tiling, explicit 72 DPI PNG density, ten-tag ICNS representation set,
and macOS 824-in-1024 content-size margins, so the result visibly drifts from
the platform standard beside every correctly generated icon.

```text
Where should the App identity icon come from?
|
+-- Vite dev/build owns the project
|     -> openTrayAppIconPlugin
|        (references/bundling.md; a thin shell over the same kernel)
|
+-- any other toolchain
|     -> generateOpenTrayAppIcon() from @opentray/icon
|
`-- no brand artwork at all
      -> omit appIcon
      -> runtime synthesizes the appName first-glyph default
         through the same kernel (generateDefaultAppIcon)
```
...
const metadata = await generateOpenTrayAppIcon({
  sourcePath: "./resources/brand-symbol.svg",
...
// metadata.appIcon is the AppIcon array: absolute file sources
// ready for createTray(..., { appIcon }).
```

Generation always emits all three platform families — one ICNS, one ICO, the
Linux PNG ladder, plus a relocatable `app-icon.json` manifest — on a
WebAssembly image stack (no sharp, no libvips, no compiler toolchain), so the
same code path runs under Node, Bun, and browsers. The cache identity covers
the source bytes, the generator implementation, the rendering recipe, the
encoder/rasterizer stack versions, and every output path; regeneration is a
no-op until one of those facts changes. Do not defeat the cache by deleting
outputs from a build script.

Composition semantics — the part hand-rolled pipelines get wrong:

| Option | Semantics |
| --- | --- |
| `sourcePath` | The brand symbol: square, transparent-margin artwork. A favicon or an already-rounded platform icon is not a valid source. |
| `composed: false` (default) | Foreground pixels are never recolored. The background — black, white, or transparent — is auto-selected from the artwork's own luminance, and the composition is clipped to the squircle mask. |
| `composed: true` | The source already bakes in background plus squircle; pixels pass through verbatim with no re-tiling. |
| `macosSourcePath` | A separate macOS content source: ICNS encodes from it while ICO and Linux use `sourcePath`. |

### The shared default

An omitted `appIcon` never inherits tray artwork. The runtime synthesizes the
default through `generateDefaultAppIcon` — the application name's first glyph
on the same squircle tile — and create-opentray uses the same generator for its
glyph fallback. Brand icons and default icons therefore share one visual
standard; a hand-rolled catalog is the only way to break it.

### Install alignment

`@opentray/icon` publishes on the same OpenTray package line as `opentray` and
the official extensions. Install the same version — or the same protocol-line
tag — across the set; read [`versioning.md`](versioning.md) before pinning
icon generation separately from the runtime.

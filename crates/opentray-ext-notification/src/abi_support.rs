//! Notification artifact identity and structured ABI error transport.
//!
//! Mirrors the dialog/sound crate family's support surface with the
//! notification identity: extensionName `notification`, contract
//! fingerprint `opentray-ext-notification-contract-1`. The facade identity
//! files are staged by `build.rs` (real `packages/ext-notification` bytes
//! when the concurrent facade batch has landed, the byte-equivalent
//! design-frozen fallback otherwise).
...
pub unsafe extern "C" fn opentray_ext_manifest(out: *mut ExtOwnedBytes) -> ExtResultCode {
    ...
    let manifest = match build_embedded_extension_manifest(
        ...
        include_str!(concat!(
            env!("OUT_DIR"),
            "/ext_notification_facade_package.json"
        )),
        include_str!(concat!(env!("OUT_DIR"), "/ext_notification_contract.json")),

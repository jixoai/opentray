//! Sound artifact identity and structured ABI error transport.
//!
//! Mirrors the dialog crate's support surface (add-ext-dialog batch B)
//! with the sound identity: extensionName `sound`, contract fingerprint
//! `opentray-ext-sound-contract-1`. The facade identity files are staged
//! by `build.rs` (real `packages/ext-sound` bytes when the concurrent
//! facade batch has landed, the byte-equivalent design-frozen fallback
//! otherwise).
...
pub unsafe extern "C" fn opentray_ext_manifest(out: *mut ExtOwnedBytes) -> ExtResultCode {
    ...
    let manifest = match build_embedded_extension_manifest(
        "sound",
        include_str!(concat!(env!("OUT_DIR"), "/ext_sound_facade_package.json")),
        include_str!(concat!(env!("OUT_DIR"), "/ext_sound_contract.json")),

...
/// message (toolbar navigate/back/forward) stalled until some unrelated
/// activation-time command happened to flush the outbox. Page-originated
/// channel commands now push drained host events through the instance's
/// EventPort immediately: the same `ExtEvent` frame shape the response
/// path produced, routed to the owning session without any command in
/// flight. Records the port cannot guarantee return to the front of the
/// host outbox for the unchanged response-flush fallback — user data is
/// never dropped.
pub(super) fn submit_host_channel_events(bridge: &Rc<RefCell<NavigatorWindowBridge>>) {
    ...
    for (owner, data) in events {
        let status = port_state.submit_channel_event(&owner.tray_id, &data);
        if webview_debug_enabled() {
            eprintln!(
                "opentray-ext-webview host channel submit: tray={} type={} -> {:?}",
                owner.tray_id,
                data["type"].as_str().unwrap_or("unknown"),
                status
            );
        }
        if status == crate::event_port::ChannelSubmit::Retain {
            retained.push(data);
        }
    }

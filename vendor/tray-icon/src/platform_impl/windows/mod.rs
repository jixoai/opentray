...
unsafe fn register_tray_icon(
    ...
    // Registration and Shell_NotifyIconGetRect must address the same shell identity. OpenTray's
    // durable app/tray identity is not available at this dependency boundary, so keep the crate's
    // native `(HWND, uID)` identity instead of inventing a process-local GUID.
    let mut nid = NOTIFYICONDATAW {
        uFlags: flags,
        hWnd: hwnd,
        uID: tray_id,

...
Windows white-block reset law: do not expose white-block repair as `navigator.opentrayWindow` methods or route it through `navigator.opentray.ipc.postMessage(...)`; IPC remains app message transport. The manual page-side escape hatch is `navigator.opentray.execCommand("clearWhiteBlock")`, a fire-and-forget command channel for low-level runtime commands that do not return values and still requires the page to have window native API access. On Windows, `clearWhiteBlock` uses the proven `SW_SHOWMINNOACTIVE -> SW_RESTORE` shell-state reset, because repaint, hide/show, frame nudges, decoration nudges, and DWM cloak/uncloak did not clear the artifact reliably. The runtime may automatically apply the same reset after first show, once after an observed native resize completes at `WM_EXITSIZEMOVE`, and after soft-resize capture release when auto clear is enabled and the background is transparent, semantic blur, or a platform material. `WM_SIZE` during a native interaction only synchronizes surfaces and records that terminal repair is eligible; it must never perform the shell-state reset. A visible non-maximized frameless window queues one private HWND message after frameless projection, style projection, or soft-resize capture release, then rechecks eligibility before clearing; this covers opaque backing where native-chrome residue is unrelated to background family. A retained `close() -> toVisible()` reveal instead uses one cancelable 100ms HWND timer, because a queue turn alone can precede DWM/WebView2 composition; hide cancels the timer and its callback rechecks normal visible state. Frameless application-level soft resize owns pointer capture and MUST NOT run this shell-state reset until that terminal completion point. The reset's temporary minimize/restore MUST NOT emit public operational-visibility changes; Windows projects `visible` from the shared command and terminal-`WM_SIZE` state tracker. Default auto clear is enabled unless `OPENTRAY_WINDOWS_AUTO_CLEAR_WHITE_BLOCK=0|false|off|no`. Do not include Windows energy-saver state in this decision: it is not a stable capability predicate for visual correctness. Maximized windows are a known hard limitation: all tested shell reset variants failed to clear the maximized artifact, so automatic repair must skip maximized state instead of fighting the user with maximize/restore loops.
...
Use `pnpm --filter opentray example:win32-bug` only for Windows composition research. It reuses every control from the Window card, then adds a manual `clearWhiteBlock` control and a reversible one-pixel resize pulse. Its source host enables `OPENTRAY_WINDOWS_COMPOSITION_DIAGNOSTICS=1` and forces `OPENTRAY_WINDOWS_AUTO_CLEAR_WHITE_BLOCK=0`, so the pulse does not silently include automatic shell recovery. The manual command remains the shell-state control. Frameless mode uses transparent page-owned chrome: empty titlebar space starts `startAppRegionDrag`, while the diagnostic operator can show or hide self-drawn minimize, maximize or restore, and close controls. Framed mode must never duplicate native caption controls. Native records describe requested host policy and operation timing but cannot prove pixels were cleared. Do not use its existence to claim that shell-state recovery, a frame change, `SWP_NOCOPYBITS`, controller reparenting, or composition hosting has solved residue; require the user to compare opaque/Mica/Acrylic, flash, focus, input, and the visible artifact in the same retained session.

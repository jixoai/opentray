...
Self-drawn controls law: whether the page should draw its own close/minimize/maximize buttons depends on whether **native window controls are visible**, which is governed by `style.frameless` — not by `windowControlsOverlay`. A frameless (`NSWindowStyleMask::Borderless` / Win32 popup) window has no native title bar and therefore no native controls, so the page MUST draw its own and wire them to `bridge.close()/minimize()/maximize()/restore()`. A framed window keeps its native controls even when `windowControlsOverlay` is on (macOS `FullSizeContentView`, Windows `ExtendsContentIntoTitleBar`) — the overlay merely lets content extend behind the still-visible native buttons, so the page should NOT draw duplicate controls. Do not infer "native controls are gone" from `bridge.overlay` being absent: `overlay` is only exposed when `windowControlsOverlay: true`, so a frameless window without overlay has neither native controls nor an overlay object, yet still needs self-drawn controls. The correct page-side predicate is `capabilities.frameless === true` (or `style.frameless`), queried via `getCapabilities()` / `getStyle()`. Overlay inset avoidance (`getTitlebarAreaRect`) and self-drawn control visibility are therefore two independent concerns: pad the titlebar when `bridge.overlay` exists, and draw controls when the window is frameless.

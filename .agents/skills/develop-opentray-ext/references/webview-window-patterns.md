...
Windows overlay geometry law: `ExtendsContentIntoTitleBar(true)` and `AppWindowTitleBar.TitleBarOcclusions` do not have identical availability. Some Windows App Runtime installations can apply overlay controls while returning null/unsupported occlusion data. Treat AppWindow occlusions as the precise geometry source only; `navigator.opentrayWindow.overlay.getTitlebarAreaRect()` must fall back to Win32 caption-control metrics so page layout remains available instead of reporting overlay unsupported.

...
const killCommand = async () => {
  ...
  try { process.kill(-command.pid, "SIGTERM"); } catch { /* already gone */ }
};
...
const tray = await createTray({
  ...
  appLaunch: {
    ...
    command: process.versions.bun !== undefined ? "node" : process.execPath,
...
if (hasShell) {
  ...
  const monitor = setInterval(async () => {
    try {
      ...
      for (const port of listening) seenPorts.add(port);
...
const quit = async () => {
  ...
  if (plainWindow !== null) {
    try { await plainWindow.destroy(); } catch {}
  }
  await tray.destroy();
  await killCommand();
  process.exit(0);

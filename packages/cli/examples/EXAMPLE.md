...
`example:hostAtoms` is the manual acceptance harness for all five host-atom extensions (`@opentray/ext-clipboard`, `@opentray/ext-opener`, `@opentray/ext-notification`, `@opentray/ext-dialog`, `@opentray/ext-sound`). It is a pure tray-menu example — no WebView page: the tray icon (label `HA`) carries a submenu per extension plus one scenario per public API surface, and the primary action runs the entire suite sequentially (dialog modal/picker scenarios are interactive and skipped by `--self-test`).
...
- **Notification** — title/body/subtitle/silent presentations (darwin native subtitle vs win32 em-dash join; subtitle-only never leaves a dangling separator), exact 64/256/64 boundary payloads, over-limit and unknown-field typed rejections, `requestAuthorization`/`getAuthorizationStatus` (darwin real prompt; win32 documented always-granted), frozen backend snapshot, plus an environment probe printing the darwin carrier-signing reality (macOS 26 refuses UN authorization for ad-hoc-signed carriers — presentation requires a Developer-ID-signed carrier; the typed outcome is the expected acceptance evidence on dev carriers).
- **Dialog** — alert/confirm/messageDialog with the full option surface (buttons/default/cancel/severity/detail/suppression), pickFile single+multiple with filters and defaultPath, pickDirectory/pickSavePath with fileNameLabel, frozen backend snapshot (all interactive — click them from the tray menu).
- **Sound** — every beep kind, the common system-sound names, one platform-native name plus a guaranteed-miss typed rejection, the win32 WAV playback path, frozen backend snapshot.

...
interface TreeContextValue<T = any> {
  ...
  tree?: any
  toggleIconType?: ToggleIconType | undefined
}
  ...
  return (
    ...
    >
      {item.isFolder() &&
        (toggleIconType === "plus-minus" ? (
          item.isExpanded() ? (
            <MinusIcon aria-hidden className="text-muted-foreground size-3.5" stroke="currentColor" strokeWidth="1" />
          ) : (
            <PlusIcon aria-hidden className="text-muted-foreground size-3.5" stroke="currentColor" strokeWidth="1" />
          )
        ) : (
          <ChevronDownIcon aria-hidden className="text-muted-foreground size-4 in-aria-[expanded=false]:-rotate-90" />
        ))}
      {children ||

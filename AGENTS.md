...
- Windows cold-start construction is a separate procedure from retained style updates: create the hidden top-level HWND with `CS_HREDRAW | CS_VREDRAW` and no `CS_OWNDC`, publish host paint/material ownership, project Win32/DWM/AppWindow style, apply final initial geometry, commit the complete native client, then create and attach the alpha-capable WebView2 child. Never run the visible-frame size correction before WndProc owns the material base. Retained style updates keep the existing order: publish policy, suppress attached-surface commits, project native style, complete parent, then commit WebView2 background/bounds. `WM_WINDOWPOSCHANGED` remains position-only; `WM_SIZE` runs `host paint -> controller bounds -> WRY child bounds -> parent-position notification`.
...
- Native residue evidence law: the residue belongs to stale top-level HWND/DWM redirection content, not to DOM/WebView page pixels. A WebView restricted to one ninth of the client still leaves residue in the other eight ninths; framed and frameless windows behave the same. This evidence requires a persistent native host painter rather than a post-hoc WebView refresh.
- Windows regression-example law: `example:win32-bug` is the WebView-controlled comparator for `native-material-host-paint-probe-20260716.exe`. It enables `OPENTRAY_WINDOWS_NATIVE_MATERIAL_PROBE=1`, starts at 900x620 framed Acrylic, and renders only the centered native-probe control matrix over a fully transparent page. The environment-gated native state may switch no-paint/black/gray and Acrylic/Mica/None on one retained HWND; ordinary OpenTray windows never expose those probe overrides and keep production Black material paint.

...
- Apply Windows chrome in this order: `SetWindowLongPtr` style/ex-style, DWM non-client policy and client-surface attributes, then one non-shell `SetWindowPos(SWP_FRAMECHANGED)`. The projection itself must not depend on a synthetic resize or host rebuild. After a visible normal frameless transition completes, the existing shell-state artifact clear may remove compositor residue; never run it while soft-resize owns capture or while the window is minimized/maximized.
...
- A retained WebView tray surface uses one `primaryEvent` menu item labeled with the next action: `Show Example` when `isVisible()` is false and `Hide Example` when true. Bootstrap the first session with `show()`, reveal retained sessions with `toVisible()`, hide with `close()`, and update the menu from `visibleChange`; do not treat a local boolean as visibility authority.

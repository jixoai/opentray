...
- Apply Windows chrome in this order: `SetWindowLongPtr` style/ex-style, DWM non-client policy and client-surface attributes, then one non-shell `SetWindowPos(SWP_FRAMECHANGED)`. The projection itself must not depend on a synthetic resize or host rebuild. After a visible normal frameless transition completes, queue one HWND message and only then run the shell-state artifact clear; never run it while soft-resize owns capture or while the window is minimized/maximized.
...
- Composition investigation law: `SW_SHOWMINNOACTIVE -> SW_RESTORE` is a shell-state recovery, not a repaint primitive. The current 120ms ordinary-`WM_SIZE` throttle is explicit UX debt because it makes that recovery visible during resize. Before preserving, retuning, or reintroducing continuous clears, measure clear reason/count/cost and compare terminal-only recovery; a trailing delay is an experiment, not a default fix. The active Wry path uses windowed WebView2 controller hosting, so this is a host/composition diagnosis, not evidence against Rust or the `windows` projections.
